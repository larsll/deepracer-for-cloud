...
nameprefix=$(while read LINE; do grep "DR_LOCAL_S3_PRETRAINED_PREFIX"; done <${DR_DIR}/run.env | cut -d "=" -f2)

DATA_DIR=${DR_DIR}/data/minio/bucket/${nameprefix}
mkdir -p ${DATA_DIR} && rm -rf ${DATA_DIR} && mkdir -p ${DATA_DIR}/model && mkdir -p ${DATA_DIR}/ip
aws s3 sync s3://${TARGET_PROFILE_S3_BUCKET}/${TARGET_PROFILE_S3_PREFIX}/model/ ${DATA_DIR}/rl-deepracer-1/model/ ${OPT_DRYRUN}


MODEL_METADATA_S3_KEY="${TARGET_CUSTOMFILES_S3_KEY}/model_metadata.json"
aws s3 cp ${MODEL_METADATA_S3_KEY} ${DATA_DIR}/rl-deepracer-1/model/model_metadata.json ${OPT_DRYRUN}
REWARD_S3_KEY="${TARGET_CUSTOMFILES_S3_KEY}/reward_function.py"
aws s3 cp ${REWARD_S3_KEY} ${DATA_DIR}/rl-deepracer-1/reward_function.py ${OPT_DRYRUN}
HYPERPARAMS_S3_KEY="${TARGET_CUSTOMFILES_S3_KEY}/hyperparameters.json"
aws s3 cp ${HYPERPARAMS_S3_KEY} ${DATA_DIR}/rl-deepracer-1/ip/hyperparameters.json ${OPT_DRYRUN}
