...
## BUCKET is the fundamental input. you need to tell it where in s3 your training will be stored
## just the bucket name.  no S3:// etc
...
DR_ROBOMAKER_IMAGE=2.0.7-cpu-avx2


## check the s3 bucket location for existing training folders
...
LAST_TRAINING=$(aws s3 ls $BUCKET/rl-deepracer | sort -t - -k 3 -g | tail -n 1 | awk '{print $2}')
## drop trailing slash
...
## upload the new run.env and system.env files into your S3 bucket (same bucket identified earlier)
aws s3 cp ./run.env s3://$BUCKET/run.env
aws s3 cp ./system.env s3://$BUCKET/system.env
...
#aws s3 cp ./autorun.sh s3://$BUCKET/autorun.sh

## upload custom files -- if you dont want this, comment these lines out
aws s3 cp ./model_metadata.json s3://$BUCKET/custom_files/model_metadata.json
aws s3 cp ./reward_function.py s3://$BUCKET/custom_files/reward_function.py
aws s3 cp ./hyperparameters.json s3://$BUCKET/custom_files/hyperparameters.json


## launch an ec2
## update with your own settings, including key-name, security-group, and iam-instance-profile at a minimum
## user data includes a command to create a bucket.txt file which simply contains the name of the bucket
## this bucket name will be used as fundamental input to autorun.sh script run later on that instance
...
aws ec2 run-instances \
    ...
    --user-data "#!/bin/bash
    su -c 'git clone https://github.com/larsll/deepracer-for-cloud.git && echo "$BUCKET" > /home/ubuntu/deepracer-for-cloud/bucket.txt && /home/ubuntu/deepracer-for-cloud/bin/prepare.sh' - ubuntu"
